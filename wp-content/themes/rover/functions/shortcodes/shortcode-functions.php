...
function shortcode_accordions( $atts, $content = null)
{
	...
	foreach( $accordions_array as $accordion )
	{
		...
		$output .= '<div class="accordion-content">'. theme_shortcode_text($accordion['content']) .'</div>';
		$output .= '</li>';
	}
...
function shortcode_video($atts, $content = null)
{
	...
	if($video_ogv || $video_mp4 || $video_webm)
	{
		...
		$output .= '<video class="video-js vjs-default-skin" poster="'.$video_poster_image.'" data-aspect-ratio="1.78" data-setup="{}" controls>'."\n";
		if($video_mp4) { $output .= '<source src="'.$video_mp4.'" type="video/mp4" />'."\n"; }
		if($video_webm) { $output .= '<source src="'.$video_webm.'" type="video/webm" />'."\n"; }
		if($video_ogv) { $output .= '<source src="'.$video_ogv.'" type="video/ogg" />'."\n"; }
		$output .= '</video>';
	}
