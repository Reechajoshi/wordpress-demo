...
{	
	function theme_latest_filckr_gallery($flickr_id, $image_count) 
	{
		...
		// Get tweets
		$images = get_transient('theme_latest_filckr_gallery');
		...
		if ( $images === false ) {
			...
			if( !is_wp_error( $rss_feed ) && isset( $rss_feed ) ) {
				...
				if( count( $image ) ) {
					...
					set_transient('theme_latest_filckr_gallery', $images, 60 * 60);
