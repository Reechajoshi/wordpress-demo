	...
	if(!is_front_page()) 
	{
		...
		elseif(is_home())
		{
			if($blog_header_title) {
				...
				echo __( get_page_name($post_page_id), 'TR' );
			}
		}
		elseif(is_page())
		{
...
function theme_portfolio_classic_filter() 
{
	...
	if($portfolio_page_id) { echo '<li'.$current_class.'><a href="'.get_page_link($portfolio_page_id).'">'.__('[:en]All[:zh]所有', 'TR').'</a></li>'; }
