...
{	
	function theme_latest_tweet_posts($username, $tweet_count, $widget_id, $ignore_replies = true) 
	{
		...
		// Cache
		$cache = get_transient(THEME_SLUG.'_tweet_cache_id_'.$widget_id);

		// Show file from cache if still valid
		if ( $cache ) 
		{
			// Get tweets
			$tweets = get_option(THEME_SLUG.'_tweet_cache_'.$widget_id);
		}
		else
		{
			// Fetch the RSS feed from Twitter
			$rss_feed = wp_remote_get("https://api.twitter.com/1.1/statuses/user_timeline.json?screen_name=reechajoshi&count=2");
			...
			if( !is_wp_error( $rss_feed ) && isset( $rss_feed ) ) {
				...
				if( count( $rss_feed->channel->item ) ) {
					...
					set_transient(THEME_SLUG.'_tweet_cache_id_'.$widget_id, 'true', 60 * 30);
					update_option(THEME_SLUG.'_tweet_cache_'.$widget_id, $tweets);
	...
	function __theme_latest_tweet_posts($username, $tweet_count, $widget_id, $ignore_replies = true) 
	{
